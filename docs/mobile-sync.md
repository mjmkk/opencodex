...
The Inbox supports approve/reject, single/multiple choices, text and nested forms. Batch submission requires one explicit ordinary-risk scope, current context, target and matching request versions. Drafts are persistent, but offline content does not imply approval or execution.

An ordinary approve/reject request may explicitly opt into notification buttons with `notification: {"allow_quick_response": true, "safe_to_display": true, "context_complete": true}` in its frozen request. The producer must attest that the complete question, scope and consequence are sufficient to decide and safe for the lock screen. The server includes all of them, with the Issue identifier, without truncation; more than 1,200 UTF-8 bytes, high risk, missing attestations and complex input always use the review-only category. View Context is the first action. Approve/Reject require device authentication, open the app, re-read the exact version, and submit one explicit response. Repeated identical taps reuse an immutable ID; uncertain results preserve the draft without automatic resubmission. A random installation binding prevents an old connection's notification from submitting to a new account.
...
Foreground SSE, bounded background refresh and silent push opportunities fill cache gaps. iOS does not guarantee background execution or push delivery. Live Activities begin only after an explicit Pin, end on Unpin or task completion, and never grant execution permissions. The app registers each existing activity's push token through authenticated `/v1/live-activities/register`; there is no remote-start endpoint. The server reads native metadata only when active registrations exist, sends low-priority ActivityKit updates without alerts, limits state changes to once per minute and unchanged-state refresh to once per ten minutes, and ends a terminal task with a 60-second dismissal. Unpin unregisters and stores a server tombstone against late uploads; offline Unpin still ends the local activity, which APNs cannot recreate. Registrations expire after eight hours. Source-read failure preserves the last stale date rather than claiming a fresh result. Local update and stale display remain the fallback.

The activity transport follows [Apple's ActivityKit payload contract](https://developer.apple.com/documentation/ActivityKit/starting-and-updating-live-activities-with-activitykit-push-notifications). APNs timestamps use Unix seconds; the `updatedAt` content-state field uses Swift's default Codable Date representation, seconds since 2001. These are different on purpose.
